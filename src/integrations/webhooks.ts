...
export class Webhooks {
    ...
    private once(a: Entity<Account>, eventId: string, fn: () => void): boolean { const db = this.studio.store.db; return this.studio.store.transaction(() => { const result = db.prepare('INSERT OR IGNORE INTO webhook_events VALUES (?,?,?)').run(a.id, eventId, Date.now()); if (!result.changes)
        return false; fn(); return true; }); }
    private inbound(p: Principal, a: Entity<Account>, eventId: string, recipient: string, body: string, at: string): void {
        ...
        this.once(a, eventId, () => { const store = this.studio.store; store.create(p, a.brandId, 'inbox', { accountId: a.id, platform: a.data.platform, recipient, text: body.slice(0, 10000), at, direction: 'INBOUND', externalId: eventId, status: 'OPEN' }); const old = store.list(p, 'contact', a.brandId).find(x => x.data.accountId === a.id && x.data.recipient === recipient); const lastInboundAt = old?.data.lastInboundAt && old.data.lastInboundAt > at ? old.data.lastInboundAt : at; const data = { ...old?.data, accountId: a.id, recipient, name: old?.data.name ?? recipient, optIn: old?.data.optIn ?? false, lastInboundAt }; if (old)
            store.update(p, old.id, old.revision, data);
        else
            store.create(p, a.brandId, 'contact', data); });
    }
    private delivery(p: Principal, a: Entity<Account>, status: Bag): void { const key = `status:${status.id}:${status.status}`; this.once(a, key, () => { const s = this.studio.store; const e = s.list<Content>(p, 'content', a.brandId, 5000).find(x => x.data.accountId === a.id && x.data.publication?.externalId === String(status.id)); if (!e?.data.publication)
        return; const state = ['sent', 'delivered', 'read'].includes(status.status) ? 'PUBLISHED' : status.status === 'failed' ? 'FAILED' : e.data.status; const delivery = e.data.publication.details?.delivery; const rank: Bag = { accepted: 0, sent: 1, delivered: 2, read: 3 }; if (rank[delivery] > rank[status.status])
        return; s.update(p, e.id, e.revision, { ...e.data, status: state, publication: { ...e.data.publication, state: state === 'PUBLISHED' ? 'PUBLISHED' : state === 'FAILED' ? 'FAILED' : 'PROCESSING', details: { ...e.data.publication.details, delivery: status.status, lastStatusAt: now() } } }); s.audit(p, a.brandId, 'message.delivery', e.id, { delivery: status.status }); }); }

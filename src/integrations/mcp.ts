import type { Bag, Principal } from '../core/types.js';
...
const schema = (properties: Bag, required: string[] = Object.keys(properties)): Bag => ({ type: 'object', properties, required, additionalProperties: false });
const TOOL_DEFS = [
...
export class McpServer {
    ...
    async handle(p: Principal, request: Bag): Promise<Bag | null> {
                ...
                try {
                    switch (def.name) {
                        case 'brand_context': {
                            ...
                            result = { brand: this.studio.brand(q, a.brandId), accounts: this.studio.store.list<import('../core/types.js').Account>(q, 'account', a.brandId).map(publicAccount) };
                            break;
                        }

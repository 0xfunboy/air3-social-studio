...
import type { Account, Bag, Entity, Platform, Principal } from '../core/types.js';
import type { Http } from '../core/http.js';
import { ProviderError, jsonRequest } from '../core/http.js';
...
export class SocialOAuth {
    ...
    useSharedApp(p: Principal, provider: string): void { requireRole(p, 'admin'); requireHuman(p); assert(OAUTH[provider], 'OAUTH_PROVIDER', 'Provider non supportato'); this.studio.store.db.prepare('DELETE FROM workspace_settings WHERE workspace_id=? AND key=?').run(p.workspaceId, 'oauth:' + provider); this.studio.store.audit(p, '', 'oauth.app.override.removed', '', { provider }); }
    private endpoint(provider: string, which: 'auth' | 'token', a: Bag): string { const s = OAUTH[provider]!; if (provider === 'meta')
        assert(/^v\d+\.0$/.test(this.studio.cfg.graphVersion), 'META_VERSION', 'Configurare META_GRAPH_VERSION nelle impostazioni installazione'); if (provider === 'mastodon')
        assert(a.instance, 'INSTANCE', 'Configurare l’istanza Mastodon'); return s[which].replace('{version}', this.studio.cfg.graphVersion).replace('{instance}', a.instance ?? ''); }
    scopes(provider: string, platform: Platform): string[] {
        if (provider === 'meta') {
    ...
    async callback(req: IncomingMessage, provider: string, u: URL): Promise<{
        grantId: string;
        clear: string;
    }> {
        ...
        const row = db.prepare('SELECT * FROM oauth_states WHERE id=? AND provider=? AND expires>?').get(sid, provider, Date.now()) as Bag | undefined;
        assert(row, 'OAUTH_STATE', 'Richiesta scaduta o già utilizzata');
        const d = decrypt<Bag>(row.data, this.studio.cfg.masterKey, 'oauth:' + sid), name = 'air3_oauth_' + sid.slice(0, 16);
        ...
        const owner: Principal = { ...p, workspaceId: row.workspace_id, brandId: row.brand_id, role: 'admin' };
        ...
        if (c.scopes) {
            const needed = d.scopes.filter((x: string) => !['offline.access', 'openid', 'profile'].includes(x));
            assert(needed.every((s: string) => c.scopes.includes(s)), 'OAUTH_SCOPE', 'Permessi incompleti: riconnettere e concedere gli scope richiesti');
        }
    ...
    private async get(url: string, token: string, headers: Record<string, string> = {}): Promise<Bag> { const b = (await this.http.request(url, { headers: { ...bearer(token), ...headers } })).body; assert((!b.error || b.error.code === 'ok') && b.ok !== false, 'OAUTH_DISCOVERY', 'La lettura degli account è stata rifiutata dal provider'); return b; }
    ...
    grant(p: Principal, grantId: string): Bag { requireRole(p, 'admin'); requireHuman(p); const r = this.studio.store.db.prepare('SELECT * FROM connection_grants WHERE id=? AND workspace_id=? AND user_id=? AND expires>?').get(grantId, p.workspaceId, p.userId, Date.now()) as Bag | undefined; assert(r && (!p.brandId || p.brandId === r.brand_id), 'GRANT', 'Connessione scaduta o non autorizzata', 404); return { ...r, payload: decrypt<Bag>(r.data, this.studio.cfg.masterKey, 'grant:' + grantId) }; }
    publicGrant(p: Principal, grantId: string): Bag { const r = this.grant(p, grantId); return { id: r.id, brandId: r.brand_id, workspaceId: r.workspace_id, provider: r.provider, scopes: r.payload.scopes, scopeStatus: r.payload.scopeStatus, expires: r.expires, candidates: r.payload.candidates.map((x: Candidate) => ({ key: x.key, platform: x.platform, targetId: x.targetId, name: x.name })) }; }
    select(p: Principal, grantId: string, keys: string[]): Bag[] { return this.studio.store.transaction(() => { const r = this.grant(p, grantId); const activeApp = this.app(p.workspaceId, r.provider); for (const c of r.payload.candidates) {
    ...
    private async refresh(e: Entity<Account>, c: Bag): Promise<Bag> {
        try {
            ...
            if (provider === 'threads') {
                ...
                const body = new URLSearchParams({ grant_type: 'refresh_token', refresh_token: c.refreshToken }), headers: Record<string, string> = { 'Content-Type': 'application/x-www-form-urlencoded' };
                if (spec.basic)
                    headers.Authorization = basic(a);
                else {
                    body.set(provider === 'tiktok' ? 'client_key' : 'client_id', a.clientId);
            ...
            this.studio.store.transaction(() => {
                ...
                const p: Principal = { workspaceId: e.workspaceId, userId: 'oauth-maintenance', role: 'admin', via: 'system' };
                const siblings = c.oauthGrantId ? this.studio.store.list<Account>(p, 'account', undefined, 10000) : [latest];
    ...
    async maintain(): Promise<void> {
        ...
        try {
            const rows = this.studio.store.db.prepare("SELECT id,workspace_id,brand_id FROM entities WHERE kind='account'").all() as Bag[];
            for (const r of rows) {
    ...
    startMaintenance(): void { if (this.maintenanceTimer)
        return; this.maintenanceTimer = setInterval(() => void this.maintain().catch(() => console.error('OAuth maintenance failed')), 60000); this.maintenanceTimer.unref(); }
    ...
    async connectTelegram(p: Principal, bid: string, input: Bag): Promise<Bag> { requireRole(p, 'admin'); requireHuman(p); this.studio.scope(p, bid); const botToken = text(input.botToken, 'Bot token', 500), target = text(input.targetId, 'Chat ID o @canale', 200); const me = await this.studio.hub.direct.telegram({ botToken }, 'getMe', {}), chat = await this.studio.hub.direct.telegram({ botToken }, 'getChat', { chat_id: target }), member = await this.studio.hub.direct.telegram({ botToken }, 'getChatMember', { chat_id: chat.id, user_id: me.id }); assert(chat.type !== 'channel' || (['administrator', 'creator'].includes(member.status) && member.can_post_messages !== false), 'TELEGRAM_PERMISSION', 'Il bot deve essere amministratore con permesso di pubblicare'); assert(!['left', 'kicked', 'restricted'].includes(member.status), 'TELEGRAM_PERMISSION', 'Bot senza permessi nella chat'); const credentials = { botToken, webhookSecret: randomBytes(32).toString('base64url') }; const old = this.studio.store.list<Account>(p, 'account', bid).find(x => x.data.platform === 'telegram' && x.data.targetId === String(chat.id)); const account = this.studio.saveAccount(p, bid, { name: chat.title ?? me.username, platform: 'telegram', transport: 'direct', targetId: String(chat.id), credentials, revision: old?.revision }, old?.id); this.health(account.id, 'CONNECTED', 'Bot e permessi verificati con Telegram'); return account; }
    async disconnect(p: Principal, accountId: string): Promise<void> { requireRole(p, 'admin'); requireHuman(p); const e = this.studio.hub.account(p, accountId); const blank = encrypt({}, this.studio.cfg.masterKey, credentialAad(e.workspaceId, e.brandId, e.id)); this.studio.store.update(p, e.id, e.revision, { ...e.data, enabled: false, credential: blank }); this.health(e.id, 'DISABLED', 'Credenziali locali eliminate. La revoca globale del grant va eseguita nella console del social.'); this.studio.store.audit(p, e.brandId, 'account.disconnected', e.id); }
}

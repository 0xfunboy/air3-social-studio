import type { Account, Bag, Brand, Content, Entity, Knowledge, Media, Metrics, Principal, Receipt, SocialPost } from './types.js';
...
import type { Model } from '../agents/llm.js';
...
export class Studio {
    ...
    saveBrand(p: Principal, input: Bag, brandId?: string): Entity<Brand> {
        requireRole(p, 'admin');
        requireHuman(p);
        noSecrets(input);
    ...
    saveAccount(p: Principal, brandId: string, input: Bag, accountId?: string): Entity<Bag> {
        requireRole(p, 'admin');
        requireHuman(p);
        ...
        assert(transport === 'direct' ? CAPABILITIES[platform as Account['platform']].native.length : CAPABILITIES[platform as Account['platform']].postiz, 'TRANSPORT', 'Trasporto non disponibile per questo social');
        ...
        const data: Account = { name: text(input.name, 'nome account', 150), platform, transport, targetId, options, enabled: input.enabled !== false, credential: credentials ? encrypt(credentials, this.cfg.masterKey, credentialAad(p.workspaceId, brandId, aid)) : old!.data.credential };
    ...
    async saveKnowledge(p: Principal, brandId: string, input: Bag, documentId?: string): Promise<Entity<Knowledge>> {
        ...
        const old = documentId ? this.store.get<Knowledge>(p, documentId, 'knowledge') : undefined;
        const duplicate = this.store.list<Knowledge>(p, 'knowledge', brandId).find(d => d.id !== old?.id && d.data.title === input.title && jaccard(d.data.text, String(input.text ?? '')) > 0.98);
    ...
    contentInput(p: Principal, brandId: string, b: Bag): Content {
        ...
        const claims = (b.claims ?? []).map((c: Bag) => ({ text: text(c.text, 'claim', 3000), sourceIds: strings(c.sourceIds ?? [], 'fonti', 30) }));
    ...
    async review(p: Principal, contentId: string): Promise<Entity<Content>> {
        requireRole(p, 'editor');
        ...
        const result = this.store.update(p, e.id, e.revision, { ...e.data, status: review.passed ? 'WAITING_APPROVAL' as const : 'REVIEW_FAILED' as const, review, approval: undefined });
    ...
    approve(p: Principal, contentId: string, revision: number, options: Bag = {}): Entity<Content> {
        ...
        const data: Content = { ...e.data, status: 'APPROVED' as const, approval: { userId: p.userId, at: now(), digest: this.digest(p, e), overrideReason: options.reason || undefined } };
    ...
    schedule(p: Principal, contentId: string, revision: number, at: string): Entity<Content> {
        ...
        const result = this.store.transaction(() => { const saved = this.store.update(p, e.id, e.revision, { ...e.data, status: 'SCHEDULED' as const, scheduleAt: new Date(dueAt).toISOString() }); this.store.enqueue(p, e.brandId, 'publish', e.id, { digest: e.data.approval!.digest }, dueAt, `publish:${e.id}:${saved.revision}`); return saved; });
        this.store.audit(p, e.brandId, 'content.scheduled', e.id, { at });
        return result;
    }
    cancel(p: Principal, contentId: string, revision: number): Entity<Content> { requireRole(p, 'editor'); const e = this.content(p, contentId); assert(e.data.status === 'SCHEDULED', 'STATE', 'Solo una programmazione non ancora partita può essere annullata'); return this.store.transaction(() => { const leased = this.store.db.prepare("SELECT id FROM jobs WHERE entity_id=? AND kind='publish' AND state='LEASED'").get(e.id); assert(!leased, 'PUBLISHING', 'Il worker ha già preso in carico il post', 409); this.store.db.prepare("UPDATE jobs SET state='CANCELLED' WHERE entity_id=? AND state='QUEUED' AND kind='publish'").run(e.id); return this.store.update(p, e.id, revision, { ...e.data, status: 'DRAFT' as const, approval: undefined, scheduleAt: undefined }); }); }
    reject(p: Principal, contentId: string, revision: number, reason: string): Entity<Content> { requireRole(p, 'approver'); requireHuman(p); const e = this.content(p, contentId); assert(MUTABLE.includes(e.data.status), 'STATE', 'Contenuto non rifiutabile in questo stato'); text(reason, 'motivo', 2000); this.store.audit(p, e.brandId, 'content.rejected', e.id, { reason }); return this.store.update(p, e.id, revision, { ...e.data, status: 'REJECTED' as const, approval: undefined }); }
    reconcile(p: Principal, contentId: string, revision: number, result: Bag): Entity<Content> { requireRole(p, 'admin'); requireHuman(p); const e = this.content(p, contentId); assert(['UNCERTAIN', 'PROCESSING'].includes(e.data.status), 'STATE', 'Solo esiti incerti o in elaborazione'); const evidence = text(result.evidence, 'evidenza di verifica esterna', 3000); assert(result.published === true || result.published === false, 'EVIDENCE', 'Indicare esito verificato'); const receipt: Receipt | undefined = result.published ? { state: 'PUBLISHED', externalId: text(result.externalId, 'ID reale', 1000), url: result.url || undefined, details: { reconciledBy: p.userId, evidence } } : undefined; const saved = this.store.update(p, e.id, revision, { ...e.data, status: result.published ? 'PUBLISHED' as const : 'DRAFT' as const, publication: receipt, approval: undefined }); this.store.audit(p, e.brandId, 'publication.reconciled', e.id, { evidence, published: result.published }); return saved; }
    enqueueGeneration(p: Principal, contentId: string, revision: number, mode = 'all'): Entity<Content> { requireRole(p, 'editor'); assert(this.model.available, 'MODEL_REQUIRED', 'Configurare il modello prima di generare'); const e = this.content(p, contentId); assert(MUTABLE.includes(e.data.status), 'STATE', 'Contenuto non rigenerabile'); assert(['all', 'copy', 'visual'].includes(mode), 'MODE', 'Modalità non valida'); return this.store.transaction(() => { const saved = this.store.update(p, e.id, revision, { ...e.data, status: 'GENERATING' as const, approval: undefined, options: { ...e.data.options, consent: false } }); this.store.enqueue(p, e.brandId, 'generate', e.id, { mode }, Date.now(), `generate:${e.id}:${saved.revision}`); return saved; }); }
    ...
    async generate(p: Principal, contentId: string, mode: string): Promise<void> {
        ...
        let strategy = e.data.strategy, copy: Bag | undefined, creative = e.data.creative;
        if (mode !== 'visual') {
            ...
            const allowed = new Set((context.sources as Bag[]).map(x => x.id));
            for (const claim of copy.claims)
                for (const sid of claim.sourceIds)
                    assert(allowed.has(sid), 'HALLUCINATED_SOURCE', 'Il copywriter ha inventato un riferimento fonte');
            e = this.store.update(p, e.id, e.revision, { ...e.data, title: copy.title, text: copy.text, hashtags: copy.hashtags, claims: copy.claims, sourceIds: [...allowed] as string[], strategy, generatedBy: this.model.name, options: { ...e.data.options, slides: copy.slides, videoScript: copy.videoScript, voiceoverScript: copy.voiceoverScript, openQuestions: copy.openQuestions, consent: false } });
        }
        if (mode !== 'copy' && ['image', 'carousel', 'video', 'reel', 'story'].includes(e.data.format)) {
            ...
            if (e.data.options.generateVisual !== false) {
                let backgroundId = e.data.options.backgroundAssetId as string | undefined;
                ...
                const slides: Bag[] = Array.isArray(e.data.options.slides) && e.data.options.slides.length ? e.data.options.slides : [{ headline: creative.headline, body: '' }];
                const toRender = e.data.format === 'carousel' || ['video', 'reel'].includes(e.data.format) ? slides.slice(0, 10) : slides.slice(0, 1);
                const assets: Entity<Asset>[] = [];
        ...
        e = this.store.update(p, e.id, e.revision, { ...e.data, status: 'GENERATED' as const });
        ...
        for (let attempt = 0; attempt < b.policy.maxAutoRevisions && !e.data.review?.passed && mode !== 'visual'; attempt++) {
            ...
            e = this.store.update(p, e.id, e.revision, { ...e.data, title: copy.title, text: copy.text, hashtags: copy.hashtags, claims: copy.claims, sourceIds: [...allowed] as string[], status: 'GENERATED' as const });
            e = await this.review(p, e.id);
        }
        ...
        if (eligible) {
            ...
            e = this.store.update(p, e.id, e.revision, { ...e.data, status: 'APPROVED' as const, approval });
    ...
    convertPlan(p: Principal, planId: string): Entity<Content>[] {
        requireRole(p, 'editor');

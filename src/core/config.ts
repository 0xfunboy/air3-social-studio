...
export function config(env: NodeJS.ProcessEnv = process.env): Config {
    ...
    assert(['https:', 'http:'].includes(url.protocol) && !url.username && !url.password && url.pathname === '/', 'CONFIG', 'BASE_URL deve essere una origin http(s)');

...
export class Store {
 ...
 PRAGMA user_version=1;`);
    ...
    maybe<T = Bag>(p: Pick<Principal, 'workspaceId' | 'brandId'>, entityId: string, kind?: string): Entity<T> | undefined { try {
    ...
    claim(at = Date.now()): Job | undefined { return this.transaction(() => { const r = this.db.prepare("SELECT * FROM jobs WHERE state='QUEUED' AND due_at<=? ORDER BY due_at,id LIMIT 1").get(at) as Bag | undefined; if (!r)
        return; const token = id(); this.db.prepare("UPDATE jobs SET state='LEASED',attempts=attempts+1,lease_until=?,lease_token=? WHERE id=? AND state='QUEUED'").run(at + 180000, token, r.id); return this.decodeJob({ ...r, state: 'LEASED', attempts: r.attempts + 1, lease_until: at + 180000, lease_token: token }); }); }
    ...
    recover(at = Date.now()): void { this.transaction(() => { const rows = this.db.prepare("SELECT * FROM jobs WHERE state='LEASED' AND lease_until<?").all(at) as Bag[]; for (const r of rows) {

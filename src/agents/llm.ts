...
export interface Model {
    ...
    generate(system: string, input: Bag, schema: Bag, images?: {
        mime: string;
        data: string;
    }[], role?: string): Promise<Bag>;
}
export class LanguageModel implements Model {
    constructor(private cfg: Config, private http: Http) { }
    get available(): boolean { return !!this.cfg.llmModel; }
    get name(): string { return this.cfg.llmModel; }
    async generate(system: string, input: Bag, schema: Bag, images: {
        mime: string;
        data: string;
    }[] = [], role = ''): Promise<Bag> {
        ...
        if (this.cfg.llmProvider === 'gemini') {
            ...
            result = r.body.candidates?.[0]?.content?.parts?.filter((p: any) => p.text).map((p: any) => p.text).join('');
        }
        else {
            const content: Bag[] = [{ type: 'text', text: JSON.stringify(input) }, ...images.map(x => ({ type: 'image_url', image_url: { url: `data:${x.mime};base64,${x.data}` } }))];
...
export function validateSchema(value: any, schema: Bag, path = '$'): void {
    if (schema.enum)
    ...
    if (schema.type === 'object') {
        ...
        for (const [k, s] of Object.entries(schema.properties ?? {}))
            if (value[k] !== undefined)
                validateSchema(value[k], s as Bag, path + '.' + k);
    }

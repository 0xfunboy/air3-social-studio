...
export async function readBody(req: IncomingMessage, max = 1500000): Promise<Buffer> {
    let n = 0;
    const chunks: Buffer[] = [];
...
function send(res: ServerResponse, value: unknown, status = 200): void { const raw = JSON.stringify(value); res.writeHead(status, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' }); res.end(raw); }
function jsonBody(raw: Buffer): Bag {
    try {
...
function safeKnowledge(e: import('../core/types.js').Entity<Knowledge>): Bag { const { chunks, ...data } = e.data; return { ...e, data: { ...data, chunkCount: chunks.length } }; }
export function createApp(studio: Studio, auth: Auth, webRoot = resolve('web')): Server {
        ...
        try {
            ...
            if (path.startsWith('/api/') || path === '/mcp') {
                ...
                if (path === '/api/password' && method === 'POST') {
                    requireHuman(p);
                    const b = jsonBody(await readBody(req, 10000)), row = studio.store.db.prepare('SELECT password FROM users WHERE id=?').get(p.userId) as Bag;
                ...
                if (bidMatch) {
                    ...
                    const collections: Record<string, string> = { accounts: 'account', contents: 'content', knowledge: 'knowledge', campaigns: 'campaign', assets: 'asset', inbox: 'inbox', metrics: 'metric', insights: 'insight', plans: 'plan', prompts: 'prompt', executions: 'execution', contacts: 'contact' };
                    if (collections[tail] && method === 'GET') {
                        let rows: any[] = studio.store.list(scoped, collections[tail]!, bid, tail === 'executions' ? 100 : 1000);
                        if (tail === 'accounts')
                ...
                if (item) {
                    ...
                    const kinds: Bag = { contents: 'content', accounts: 'account', knowledge: 'knowledge', campaigns: 'campaign', insights: 'insight', inbox: 'inbox', executions: 'execution', plans: 'plan' };
                    const e = studio.store.get(p, itemId, kinds[type]), scoped = studio.scope(p, e.brandId);
                    if (!action && method === 'GET') {
                        if (type === 'accounts')
                            send(res, publicAccount(e as any));
                    ...
                    if (type === 'accounts') {
                        const a = e as import('../core/types.js').Entity<Account>;
                    ...
                    if (type === 'contents' && method === 'POST') {
                        ...
                        if (action === 'generate')
                            ...
                            result = ids.map(accountId => { const a = studio.hub.account(scoped, accountId); const original = (e as import('../core/types.js').Entity<Content>).data; const format = a.data.transport === 'direct' && !CAPABILITIES[a.data.platform].native.includes(original.format) ? CAPABILITIES[a.data.platform].native[0] : original.format; return studio.saveContent(scoped, e.brandId, { ...original, accountId, format, media: format === 'text' ? [] : original.media, options: { ...original.options, consent: false } }); });
                        }
                        else
            ...
            const assets: Record<string, string> = { '/app.js': 'text/javascript; charset=utf-8', '/style.css': 'text/css; charset=utf-8', '/brand.js': 'text/javascript; charset=utf-8', '/favicon.svg': 'image/svg+xml', '/assets/mark.svg': 'image/svg+xml', '/assets/logo-light.svg': 'image/svg+xml', '/assets/logo-dark.svg': 'image/svg+xml' };

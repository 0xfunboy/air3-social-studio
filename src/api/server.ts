...
export async function readBody(req: IncomingMessage, max = 1500000): Promise<Buffer> { let n = 0; const chunks: Buffer[] = []; for await (const chunk of req) {
    n += chunk.length;
...
function jsonBody(raw: Buffer): Bag { try {
    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            ...
            const files: Record<string, {
                file: string;
                mime: string;
            }> = { '/': { file: 'index.html', mime: 'text/html; charset=utf-8' }, '/app.js': { file: 'app.js', mime: 'text/javascript; charset=utf-8' }, '/style.css': { file: 'style.css', mime: 'text/css; charset=utf-8' } };
            const f = files[path];

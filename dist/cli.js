...
try {
    ...
    if (command === 'doctor') {
        ...
        const checks = { https: app.cfg.baseUrl.startsWith('https://'), modelConfigured: app.studio.model.available, renderer: renderer.ffmpeg && renderer.font, siteAdmin: Number(app.store.db.prepare('SELECT count(*) n FROM site_admins').get().n) > 0, privacy: !!publicConfig.privacyUrl, terms: !!publicConfig.termsUrl, support: !!publicConfig.supportEmail, registrationEmail: !publicConfig.registration || app.settings.mailEnabled };
        let envPrivate = true;
        try {
        ...
        console.log(JSON.stringify({ version: '0.2.0', node: process.version, sqlite: true, checks, envPrivate, localReadiness: ready ? 'PASS' : 'ACTION_REQUIRED', externalProviderVerification: 'NOT_PERFORMED_BY_DOCTOR', model: { configured: app.studio.model.available, name: app.studio.model.name }, embeddings: app.studio.rag.embedder.enabled, renderer, baseUrl: app.cfg.baseUrl, accounts: app.store.db.prepare("SELECT count(*) n FROM entities WHERE kind='account'").get().n, notes: 'Il controllo locale non certifica OAuth, app review, consegna email, DNS/TLS o pubblicazione live.' }, null, 2));
        ...
        const user = app.store.db.prepare('SELECT id FROM users WHERE email=?').get(email);

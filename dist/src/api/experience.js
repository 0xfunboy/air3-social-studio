...
import { assert, AppError, text, object } from '../core/util.js';
...
export class ExperienceApi {
    studio;
    auth;
    constructor(studio, auth) {
        this.studio = studio;
        this.auth = auth;
    }
    async callback(req, res, u) {
        ...
        try {
            ...
            if (provider === 'google') {
                ...
                const r = await oauth.callback(req, provider, u), row = this.studio.store.db.prepare('SELECT workspace_id FROM connection_grants WHERE id=?').get(r.grantId);
    ...
    async protected(req, res, u, p) {
        ...
        if (path === '/api/admin/installation/test-email' && method === 'POST') {
            ...
            const user = this.studio.store.db.prepare('SELECT email FROM users WHERE id=?').get(p.userId);
        ...
        if (path === '/api/admin/invitations') {
            ...
            if (method === 'GET')
                ...
                const b = await body(req);
                send(res, await identity.invite(p, text(b.email, 'email', 300), b.role), 201);
            }
            else
                return false;
            return true;
        ...
        if (path === '/api/admin/site' && method === 'GET') {
            this.auth.requireSiteAdmin(p);
            send(res, { users: this.studio.store.db.prepare('SELECT u.id,u.email,u.created_at,COALESCE(f.disabled,0) disabled,COALESCE(f.verified,1) verified,CASE WHEN a.user_id IS NULL THEN 0 ELSE 1 END site_admin FROM users u LEFT JOIN user_flags f ON f.user_id=u.id LEFT JOIN site_admins a ON a.user_id=u.id').all(), counts: { workspaces: this.studio.store.db.prepare('SELECT count(*) n FROM workspaces').get().n, brands: this.studio.store.db.prepare("SELECT count(*) n FROM entities WHERE kind='brand'").get().n }, audit: this.studio.store.db.prepare("SELECT actor,action,at FROM audit WHERE brand_id='' ORDER BY at DESC LIMIT 100").all() });
            return true;
        }
        ...
        if (start && method === 'POST') {
            ...
            const r = oauth.start(p, start[1], start[2]);
            res.setHeader('Set-Cookie', r.cookie);
            send(res, { url: r.url });
            return true;
        }

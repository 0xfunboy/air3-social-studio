...
export class Webhooks {
    ...
    account(accountId) { const row = this.studio.store.db.prepare("SELECT workspace_id,brand_id FROM entities WHERE id=? AND kind='account'").get(accountId); assert(row, 'NOT_FOUND', 'Webhook non trovato', 404); const p = { workspaceId: row.workspace_id, brandId: row.brand_id, userId: 'webhook', role: 'admin', via: 'system' }; const a = this.studio.hub.account(p, accountId); assert(a.data.enabled && a.data.transport === 'direct', 'WEBHOOK', 'Account webhook non attivo', 403); return { p, a, c: this.studio.hub.credentials(a) }; }
    challenge(accountId, q) { const { c } = this.account(accountId); assert(c.webhookVerifyToken && q.get('hub.mode') === 'subscribe' && equal(q.get('hub.verify_token') ?? '', c.webhookVerifyToken), 'WEBHOOK_VERIFY', 'Token verifica non valido', 403); return q.get('hub.challenge') ?? ''; }
    async receive(accountId, headers, raw) {
    ...
    inbound(p, a, eventId, recipient, body, at) {
    ...
    delivery(p, a, status) {
        ...
        this.once(a, key, () => {
            ...
            const e = s.list(p, 'content', a.brandId, 5000).find(x => x.data.accountId === a.id && x.data.publication?.externalId === String(status.id));
    ...
    async telegram(p, a, c, b) {
        ...
        if (query) {
            ...
            const membership = uid ? s.db.prepare('SELECT role FROM memberships WHERE user_id=? AND workspace_id=?').get(uid, a.workspaceId) : undefined;
            ...
            try {
                ...
                const actor = { ...p, userId: uid, role: membership.role, via: 'telegram' };

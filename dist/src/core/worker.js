...
export class Worker {
    ...
    async tick() {
        ...
        heartbeat.unref();
        const p = { userId: 'worker', workspaceId: job.workspaceId, brandId: job.brandId, role: 'admin', via: 'system' };
        try {
            ...
            let state = 'FAILED';
            const content = store.maybe(p, job.entityId, 'content');
            const inFlight = job.kind === 'publish' && content?.data.status === 'PUBLISHING';
            if (inFlight && (!(err instanceof ProviderError) || err.ambiguous)) {
                state = 'UNCERTAIN';
                if (content)
                    store.update(p, content.id, content.revision, { ...content.data, status: 'UNCERTAIN' });
            }
                ...
                store.update(p, content.id, content.revision, { ...content.data, status: 'FAILED' });
    ...
    async execute(p, j) {
        ...
        if (j.kind === 'notify') {
            ...
            if (!['WAITING_APPROVAL', 'REVIEW_FAILED'].includes(e.data.status))
                return 'CANCELLED';
            const a = store.list(p, 'account', j.brandId).find(x => x.data.platform === 'telegram' && x.data.enabled && x.data.options.approvalChatId && x.data.transport === 'direct');
            if (!a)
                return 'SKIPPED';
            const buttons = [[{ text: 'Apri contenuto', url: s.cfg.baseUrl + '/#content/' + e.id }]];
        ...
        if (j.kind === 'publish') {
            ...
            let current = store.update(p, e.id, e.revision, { ...e.data, status: 'PUBLISHING' });
        ...
        if (j.kind === 'poll') {
            if (e.data.status !== 'PROCESSING' || !e.data.publication)
                return 'CANCELLED';
            if (j.attempts > 180) {
                store.update(p, e.id, e.revision, { ...e.data, status: 'UNCERTAIN' });

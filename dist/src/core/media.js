...
const extensions = { 'image/png': '.png', 'image/jpeg': '.jpg', 'image/webp': '.webp', 'video/mp4': '.mp4', 'audio/mpeg': '.mp3', 'audio/wav': '.wav' };
export function sniff(buffer, mime) {
    if (mime === 'image/png')
...
export class MediaService {
    ...
    constructor(store, cfg, http) {
        ...
        this.cfg = cfg;
        this.http = http;
        this.directory = join(cfg.dataDir, 'assets');
    }
    async save(p, brandId, buffer, mime, name, generated = false, alt = '') {
    ...
    asset(p, assetId) { return this.store.get(p, assetId, 'asset'); }
    async bytes(p, assetId) { return readFile(join(this.directory, this.asset(p, assetId).data.file)); }
    path(p, assetId) { return join(this.directory, this.asset(p, assetId).data.file); }
    url(p, assetId, seconds = 86400) { const a = this.asset(p, assetId); const expires = Math.floor(Date.now() / 1000) + Math.min(seconds, 604800); const token = hmac(`${a.workspaceId}:${a.brandId}:${assetId}:${expires}`, this.cfg.masterKey); return `${this.cfg.baseUrl}/media/${assetId}?w=${a.workspaceId}&b=${a.brandId}&e=${expires}&s=${token}`; }
    signed(assetId, q) { const workspaceId = q.get('w') ?? '', brandId = q.get('b') ?? '', expires = Number(q.get('e')); assert(Number.isInteger(expires) && expires >= Date.now() / 1000 && expires <= Date.now() / 1000 + 604800, 'MEDIA_EXPIRED', 'Link media scaduto', 403); assert(equal(q.get('s') ?? '', hmac(`${workspaceId}:${brandId}:${assetId}:${expires}`, this.cfg.masterKey)), 'MEDIA_SIGNATURE', 'Link media non valido', 403); const asset = this.store.get({ workspaceId, brandId }, assetId, 'asset'); return { path: join(this.directory, asset.data.file), asset }; }
    async hydrate(p, media) {
        return media.map(m => {
            if (!m.id) {
    ...
    async generateBackground(p, brandId, prompt, references = []) {
        ...
        const part = r.body.candidates?.[0]?.content?.parts?.find((x) => x.inlineData?.data)?.inlineData;
    ...
    async render(p, brandId, brand, headline, body, options = {}) {
    ...
    async slideshow(p, brandId, assetIds, audioId) {
...
export function wrap(input, columns, maxLines) {
...
export function templateLayout(headline, body, width, height) {

...
export class Settings {
    ...
    apply(startup = false) {
        ...
        const map = { BASE_URL: 'baseUrl', LLM_BASE_URL: 'llmBase', LLM_API_KEY: 'llmKey', LLM_MODEL: 'llmModel', LLM_PROVIDER: 'llmProvider', EMBEDDING_BASE_URL: 'embeddingBase', EMBEDDING_API_KEY: 'embeddingKey', EMBEDDING_MODEL: 'embeddingModel', GEMINI_API_KEY: 'geminiKey', GEMINI_IMAGE_MODEL: 'imageModel', META_GRAPH_VERSION: 'graphVersion', LINKEDIN_VERSION: 'linkedinVersion' };
        for (const [k, dest] of Object.entries(map))
            if (v[k] !== undefined && (startup || !ENV_FIELDS[k]?.restart))
                this.cfg[dest] = String(v[k]).replace(dest.endsWith('Base') || dest === 'baseUrl' ? /\/$/ : /$^/, '');
    }
    env(secrets = false) { const v = this.values(); return '# Generated by AIR3 Social Studio. Do not commit. Account tokens remain encrypted in SQLite.\n' + Object.keys(ENV_FIELDS).map(k => `${k}=${JSON.stringify(ENV_FIELDS[k]?.secret && !secrets && v[k] ? '[REDACTED]' : v[k] ?? '')}`).join('\n') + '\n'; }
    writeGenerated() { mkdirSync(this.cfg.dataDir, { recursive: true, mode: 0o700 }); const target = join(this.cfg.dataDir, 'runtime.generated.env'), tmp = target + '.' + randomBytes(8).toString('hex'); writeFileSync(tmp, this.env(true), { mode: 0o600, flag: 'wx' }); renameSync(tmp, target); chmodSync(target, 0o600); }
    onboarding(p) { const r = this.store.db.prepare("SELECT value FROM workspace_settings WHERE workspace_id=? AND key='onboarding'").get(p.workspaceId); return r ? JSON.parse(r.value) : { step: 0, completed: false }; }
    saveOnboarding(p, b) { assert(p.role === 'admin' && p.via === 'session', 'FORBIDDEN', 'Solo amministratore workspace', 403); const step = Number(b.step ?? 0); assert(Number.isInteger(step) && step >= 0 && step <= 4, 'STEP', 'Step non valido'); const v = { step, completed: b.completed === true, updatedAt: now() }; this.store.db.prepare("INSERT INTO workspace_settings VALUES (?,'onboarding',?) ON CONFLICT(workspace_id,key) DO UPDATE SET value=excluded.value").run(p.workspaceId, JSON.stringify(v)); return v; }
}

...
export function getMemberPermissions(store, workspaceId, userId, role) {
    const base = ROLE_BASE_PERMISSIONS[role] ? [...ROLE_BASE_PERMISSIONS[role]] : [];
    const row = store.db.prepare('SELECT permissions FROM member_permissions WHERE workspace_id=? AND user_id=?').get(workspaceId, userId);
    if (row?.permissions) {
        try {
            const extra = JSON.parse(row.permissions);
            if (Array.isArray(extra)) {
                for (const p of extra) {
                    if (typeof p === 'string' && !base.includes(p))
                        base.push(p);
                }
            }
        }
...
export function hasPermission(store, p, permission) {
    if (p.role === 'admin')
        return true;
    const perms = getMemberPermissions(store, p.workspaceId, p.userId, p.role);
    return perms.includes('*') || perms.includes(permission);
}
export function requirePermission(store, p, permission) {
...
export class Auth {
    ...
    principal(req, mutating) {
        ...
        if (authorization.startsWith('Bearer ')) {
            const t = this.store.db.prepare('SELECT * FROM tokens WHERE token_hash=? AND expires>?').get(sha(authorization.slice(7)), Date.now());
    ...
    addUser(p, email, password, role) {
        ...
        return this.store.transaction(() => {
            let u = this.store.db.prepare('SELECT id FROM users WHERE email=?').get(email.toLowerCase());
            if (!u) {
                u = { id: id() };
                this.store.db.prepare('INSERT INTO users VALUES (?,?,?,?)').run(u.id, email.toLowerCase(), passwordHash(password), now());
            }
            if (role !== 'admin') {
                const prior = this.store.db.prepare('SELECT role FROM memberships WHERE user_id=? AND workspace_id=?').get(u.id, p.workspaceId);
                const admins = this.store.db.prepare("SELECT count(*) n FROM memberships WHERE workspace_id=? AND role='admin'").get(p.workspaceId);

...
export class ProviderError extends AppError {
    ...
    ambiguous;
    retryAfterMs;
    constructor(provider, retryable, ambiguous, retryAfterMs, message, status = 502) {
        super(status, 'PROVIDER', message);
...
export const jsonRequest = (body, token, headers = {}) => ({ method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}), ...headers }, body: JSON.stringify(body) });
export function apiSuccess(body, provider) {

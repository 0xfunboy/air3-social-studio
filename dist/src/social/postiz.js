...
export class PostizClient {
    ...
    base(c) { return String(c.baseUrl || 'https://api.postiz.com/public/v1').replace(/\/$/, ''); }
    headers(c) { assert(c.apiKey, 'POSTIZ_KEY', 'API key Postiz mancante'); return { Authorization: String(c.apiKey) }; }
    async integrations(c) { const r = await this.http.request(this.base(c) + '/integrations', { headers: this.headers(c) }); assert(Array.isArray(r.body), 'POSTIZ_RESPONSE', 'Elenco integrazioni Postiz non valido'); return r.body; }
    async connect(c, provider) { const r = await this.http.request(this.base(c) + '/social/' + encodeURIComponent(provider), { headers: this.headers(c) }); assert(typeof r.body.url === 'string' && r.body.url.startsWith('https://'), 'OAUTH_URL', 'Postiz non ha restituito una URL OAuth HTTPS'); return r.body.url; }
    settings(a, p) {
        const provider = CAPABILITIES[a.platform].postiz;
        assert(provider, 'POSTIZ_UNSUPPORTED', 'Questo canale non usa Postiz');
        const s = { ...(a.options.settings ?? {}), ...(p.options.settings ?? {}), __type: provider };
        if (a.platform === 'instagram')
            s.post_type = p.format === 'story' ? 'story' : 'post';
        if (a.platform === 'x')
            s.who_can_reply_post ??= 'everyone';
        if (a.platform === 'youtube')
            Object.assign(s, { title: p.title, type: p.options.privacy, selfDeclaredMadeForKids: p.options.madeForKids ? 'yes' : 'no', tags: (p.options.tags ?? []).map((v) => ({ value: v, label: v })), thumbnail: s.thumbnail ?? null });
    ...
    async publish(a, c, p) {
        ...
        const item = Array.isArray(result.body) ? result.body.find((x) => x.integration === a.targetId) : undefined;
    ...
    async poll(a, c, r) {
        ...
        const item = result.body.posts?.find((x) => x.id === r.externalId && x.integration?.id === a.targetId);
        if (!item)
            return r;

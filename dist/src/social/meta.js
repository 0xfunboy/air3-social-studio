...
export class MetaClient {
    ...
    async get(a, c, path) { return (await this.http.request(this.base(a) + path, { headers: { Authorization: `Bearer ${c.accessToken}` } })).body; }
    async post(a, c, path, body) { assert(c.accessToken, 'META_TOKEN', 'Access token Meta mancante'); const r = await this.http.request(this.base(a) + path, jsonRequest(body, c.accessToken)); apiSuccess(r.body, 'Meta'); return r.body; }
    async publish(a, c, p) {
        const target = encodeURIComponent(a.targetId), m = p.media[0];
        if (a.platform === 'whatsapp') {
            const body = { messaging_product: 'whatsapp', recipient_type: 'individual', to: p.options.recipient };

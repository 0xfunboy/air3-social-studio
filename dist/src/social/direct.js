...
export class DirectClients {
    ...
    async telegram(c, method, body) { assert(c.botToken && /^[0-9]+:[A-Za-z0-9_-]+$/.test(c.botToken), 'TELEGRAM_TOKEN', 'Bot token Telegram non valido'); const r = await this.http.request(`https://api.telegram.org/bot${c.botToken}/${method}`, jsonRequest(body)); apiSuccess(r.body, 'Telegram'); return r.body.result; }
    async publish(a, c, p) {
        let b;
        const m = p.media[0];
        switch (a.platform) {

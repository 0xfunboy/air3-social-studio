...
export class YouTubeClient {
    ...
    async publish(e, c, p) {
        ...
        const uploaded = await this.http.request(url.href, { method: 'PUT', headers: { Authorization: 'Bearer ' + c.accessToken, 'Content-Type': asset.data.mime, 'Content-Length': String(bytes.length) }, body: bytes });
    ...
    async poll(c, r) {
        const b = (await this.http.request('https://www.googleapis.com/youtube/v3/videos?' + new URLSearchParams({ part: 'status', id: r.externalId }), { headers: { Authorization: 'Bearer ' + c.accessToken } })).body;
        const s = b.items?.[0]?.status;
        assert(s, 'YOUTUBE_RESPONSE', 'Video non leggibile');
        if (['failed', 'rejected', 'deleted'].includes(s.uploadStatus))
            return { ...r, state: 'FAILED', error: 'Elaborazione video rifiutata da YouTube' };
        if (s.uploadStatus === 'processed')
            return { ...r, state: 'PUBLISHED', url: 'https://www.youtube.com/watch?v=' + encodeURIComponent(r.externalId) };
        return r;
    }
    async metrics(c, r) {
        const b = (await this.http.request('https://www.googleapis.com/youtube/v3/videos?' + new URLSearchParams({ part: 'statistics', id: r.externalId }), { headers: { Authorization: 'Bearer ' + c.accessToken } })).body;
        const values = {};
        for (const [k, v] of Object.entries(b.items?.[0]?.statistics ?? {}))
            if (Number.isFinite(Number(v)))
                values[k] = Number(v);
        return { collectedAt: now(), values, source: 'youtube-data-api' };
    }
}

...
export class TikTokClient {
    ...
    async call(c, path, payload) { assert(c.accessToken, 'TIKTOK_TOKEN', 'Token TikTok mancante'); const r = await this.http.request('https://open.tiktokapis.com/v2/' + path, jsonRequest(payload, c.accessToken)); apiSuccess(r.body, 'TikTok'); return r.body.data; }
    async creator(c) { return this.call(c, 'post/publish/creator_info/query/', {}); }
    async publish(a, c, p) {
        ...
        const info = { privacy_level: p.options.privacyLevel, disable_comment: p.options.allowComments !== true, brand_content_toggle: p.options.brandContent, brand_organic_toggle: p.options.brandOrganic };
        let b;
        if (photo)

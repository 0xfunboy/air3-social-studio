...
test('platform catalog covers requested channels and refuses silent media loss', () => {
    for (const p of ['facebook', 'instagram', 'threads', 'whatsapp', 'messenger', 'instagram-dm', 'tiktok', 'x', 'linkedin', 'linkedin-page', 'telegram', 'youtube'])
        assert(p in CAPABILITIES);
    const a = { name: 'X', platform: 'x', transport: 'direct', targetId: 'x', credential: '', enabled: true, options: {} };
    assert.throws(() => validatePost(a, { id: '1', title: 'hi', text: 'hi', format: 'text', media: [{ url: 'https://example.test/a.jpg', mime: 'image/jpeg' }], options: {} }));
    assert.equal(weightedX('https://example.com/a/very/long/link'), 23);
});
test('account ciphertext never appears in public representation; scoped decrypt works', async () => {
...
test('brand-scoped entities cannot be accessed through another brand or workspace', async () => {
    const f = await fixture();
    try {
...
test('optimistic content revisions reject stale edits', async () => {
    const f = await fixture();
    try {
...
test('machine tokens cannot approve, grant their own approval, or create admin tokens', async () => {
    const f = await fixture();
    try {
        const a = addAccount(f);
        let e = addContent(f, a.id);
        e = await f.studio.review(f.p, e.id);
        const p = { ...f.p, via: 'token', role: 'editor', brandId: f.brand.id };
...
test('mandatory brand rules cannot be overridden by a high reviewer score', async () => {
...
test('editing approved content invalidates approval and changing brand invalidates signature', async () => {
    const f = await fixture();
    try {
...
test('scheduling is durable and cancellation removes queued publication', async () => {
    const f = await fixture();
    try {
...
test('one worker publish yields one provider call and schedules 24h/72h observations', async () => {
    const f = await fixture();
    try {
...
test('ambiguous provider failure is UNCERTAIN and is never blindly retried', async () => {
    const f = await fixture();
    try {
...
test('a definite 429 is queued with backoff, not marked as successfully published', async () => {
    const f = await fixture();
    try {
...
test('expired write leases require reconciliation, read leases are safe to reclaim', async () => {
    const f = await fixture();
    try {
        const a = addAccount(f), e = await approved(f, a.id);
        f.studio.schedule(f.p, e.id, e.revision, new Date().toISOString());
...
test('RAG filters approval, scope, role, platform and expiry; repeated input is deduplicated', async () => {
    const f = await fixture();
    try {
...
test('knowledge changes invalidate an existing publication approval', async () => {
    const f = await fixture();
    try {
...
test('MCP negotiates protocol, enforces tool schemas and never exposes an approve tool', async () => {
    const f = await fixture();
    try {
        const m = new McpServer(f.studio), p = { ...f.p, role: 'editor', via: 'token', brandId: f.brand.id };
        ...
        assert(!tools.result.tools.some((x) => x.name === 'approve'));
...
test('fixture AI pipeline records role-specific calls and stops at human approval', async () => {
    const f = await fixture(true);
    try {
        const a = addAccount(f), e = addContent(f, a.id);
...
test('analyst uses real snapshot IDs and separate latest observation per post', async () => {
    const f = await fixture(true);
    try {
        const a = addAccount(f), e = await approved(f, a.id);
        f.store.update(f.p, e.id, e.revision, { ...e.data, status: 'PUBLISHED', publication: { state: 'PUBLISHED', externalId: 'real-test-id' } });

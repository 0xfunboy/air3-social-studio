...
export function hubArt() { const names = ['instagram', 'whatsapp', 'tiktok', 'x', 'pinterest', 'youtube', 'telegram', 'linkedin', 'threads', 'facebook']; return `<div class="hub-art" aria-hidden="true"><div class="orbit orbit-a"></div><div class="orbit orbit-b"></div><div class="orbit-axis"></div><div class="hub-center">${mark()}</div>${names.map((n, i) => `<div class="social-tile tile-${i}">${socialMark(n)}</div>`).join('')}<span class="handwritten art-note">Uno studio.<br>Ogni canale.<span class="pencil-stroke"></span></span><span class="handwritten art-note bottom">Dalle idee<br>all’impatto. ↗</span></div>`; }

...
for (const [platform, target, creds, reply, path, check] of directCases)
    test(`HTTP contract, not live: ${platform} text publishing`, async (t) => { const f = await fixture(); t.after(() => f.cleanup()); const a = addAccount(f, platform, 'direct', target, creds); f.http.handler = (url, init, b) => { assert.ok(url.endsWith(path), url); assert.equal(init.method, 'POST'); check(b, new Headers(init.headers)); if (platform === 'reddit') {
        const form = new URLSearchParams(String(init.body));
        assert.equal(form.get('sr'), 'testsub');
        assert.equal(form.get('kind'), 'self');
    } return response(reply, 201, platform.startsWith('linkedin') ? { 'x-restli-id': 'urn:li:share:1' } : {}); }; const r = await f.studio.hub.publish(a, post()); assert.equal(r.state, 'PUBLISHED'); assert.ok(r.externalId); assert.equal(f.http.calls.length, 1); });
...
for (const platform of ['instagram', 'threads'])
    test(`${platform}: container creation, pure GET readiness, separate final publication`, async (t) => { const f = await fixture(); t.after(() => f.cleanup()); const a = addAccount(f, platform, 'direct', 'creator'); const seen: string[] = []; f.http.handler = (u, i, b) => { seen.push(i.method ?? 'GET'); if (seen.length === 1) {
    ...
    } assert.equal(b.creation_id, 'container-1'); assert.ok(u.endsWith(platform === 'threads' ? '/threads_publish' : '/media_publish')); return response({ id: 'public-1' }); }; let r = await f.studio.hub.publish(a, post({ format: 'image', media: [image] })); assert.equal(r.state, 'PROCESSING'); r = await f.studio.hub.poll(a, r); assert.equal(r.state, 'PROCESSING'); assert.equal(r.details?.readyToPublish, true); r = await f.studio.hub.finalize(a, r); assert.equal(r.state, 'PUBLISHED'); assert.equal(r.externalId, 'public-1'); assert.deepEqual(seen, ['POST', 'GET', 'POST']); });
test('Instagram carousel children are image containers, final parent carries caption', async (t) => { const f = await fixture(); t.after(() => f.cleanup()); const a = addAccount(f, 'instagram', 'direct', 'creator'); let n = 0; f.http.handler = (_u, _i, b) => { n++; if (n < 3) {
    assert.equal(b.is_carousel_item, true);
    return response({ id: 'child' + n });
} assert.equal(b.media_type, 'CAROUSEL'); assert.deepEqual(b.children, ['child1', 'child2']); return response({ id: 'parent' }); }; const r = await f.studio.hub.publish(a, post({ format: 'carousel', media: [image, image] })); assert.equal(r.state, 'PROCESSING'); assert.equal(r.externalId, 'parent'); });
test('Facebook video remains PROCESSING until video_status ready', async (t) => { const f = await fixture(); t.after(() => f.cleanup()); const a = addAccount(f, 'facebook', 'direct', 'page'); f.http.handler = (u, i, b) => { if (i.method === 'POST') {
    assert.ok(u.endsWith('/page/videos'));
    assert.equal(b.file_url, video.url);
    return response({ id: 'video-1' });
} return response({ status: { video_status: 'ready' } }); }; const r = await f.studio.hub.publish(a, post({ format: 'video', media: [video] })); assert.equal(r.state, 'PROCESSING'); assert.equal((await f.studio.hub.poll(a, r)).state, 'PUBLISHED'); });
for (const platform of ['messenger', 'instagram-dm'])
    test(`${platform}: reply routes recipient without inventing delivery receipt`, async (t) => { const f = await fixture(); t.after(() => f.cleanup()); const a = addAccount(f, platform, 'direct', 'page'); f.http.handler = (_u, _i, b) => { assert.deepEqual(b.recipient, { id: 'recipient' }); assert.equal(b.message.text, post().text); if (platform === 'messenger')
        assert.equal(b.messaging_type, 'RESPONSE'); return response({ message_id: 'msg-1' }); }; const r = await f.studio.hub.publish(a, post({ format: 'message', options: { recipient: 'recipient' } })); assert.equal(r.details?.delivery, 'accepted-by-api'); });
...
test('TikTok video creator permissions -> init ticket -> authoritative status', async (t) => { const f = await fixture(); t.after(() => f.cleanup()); const a = addAccount(f, 'tiktok', 'direct', 'creator'); f.http.handler = (u, _i, b) => { if (u.endsWith('/creator_info/query/'))
    return response({ error: { code: 'ok' }, data: { privacy_level_options: ['SELF_ONLY'], comment_disabled: true, duet_disabled: true, stitch_disabled: true } }); if (u.endsWith('/video/init/')) {
...
} assert.equal(b.publish_id, 'ticket-1'); return response({ error: { code: 'ok' }, data: { status: 'PUBLISH_COMPLETE', publicaly_available_post_id: ['public-id'] } }); }; let r = await f.studio.hub.publish(a, post({ format: 'video', media: [video], options: ttOpts })); assert.equal(r.state, 'PROCESSING'); r = await f.studio.hub.poll(a, r); assert.equal(r.state, 'PUBLISHED'); assert.deepEqual(r.details?.publicPostIds, ['public-id']); });
...
test('TikTok photo DIRECT_POST contract, explicit privacy and commercial flags', async (t) => { const f = await fixture(); t.after(() => f.cleanup()); const a = addAccount(f, 'tiktok', 'direct', 'creator'); f.http.handler = (u, _i, b) => { if (u.endsWith('/creator_info/query/'))
    return response({ error: { code: 'ok' }, data: { privacy_level_options: ['SELF_ONLY'] } }); assert.ok(u.endsWith('/content/init/')); assert.equal(b.post_mode, 'DIRECT_POST'); assert.equal(b.media_type, 'PHOTO'); assert.deepEqual(b.source_info.photo_images, [image.url, image.url]); assert.equal(b.post_info.brand_content_toggle, false); return response({ error: { code: 'ok' }, data: { publish_id: 'photo-ticket' } }); }; assert.equal((await f.studio.hub.publish(a, post({ format: 'carousel', media: [image, image], options: ttOpts }))).state, 'PROCESSING'); });
for (const platform of ['youtube', 'linkedin', 'x'])
    test(`Postiz ${platform}: upload, provider contract and monitored publication`, async (t) => { const f = await fixture(); t.after(() => f.cleanup()); const a = addAccount(f, platform, 'postiz', 'integration'); let published = false; f.http.handler = (u, i, b) => { assert.equal(new Headers(i.headers).get('Authorization'), 'TEST_POSTIZ'); if (u.endsWith('/upload-from-url'))
        return response({ id: 'media-1', path: 'https://postiz.example.test/media.mp4' }); if (u.endsWith('/posts')) {
    ...
    } const q = new URL(u).searchParams; assert.ok(q.get('startDate')); assert.ok(q.get('endDate')); return response({ posts: [{ id: 'job-1', integration: { id: 'integration' }, state: published ? 'PUBLISHED' : 'QUEUE', releaseURL: 'https://social.example.test/post' }] }); }; const p = post({ format: platform === 'youtube' ? 'video' : 'image', media: [platform === 'youtube' ? video : image], options: { madeForKids: false, privacy: 'private', tags: [] } }); let r = await f.studio.hub.publish(a, p); assert.equal(r.state, 'PROCESSING'); assert.equal((await f.studio.hub.poll(a, r)).state, 'PROCESSING'); published = true; r = await f.studio.hub.poll(a, r); assert.equal(r.state, 'PUBLISHED'); assert.equal(r.url, 'https://social.example.test/post'); });
